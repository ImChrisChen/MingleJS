...
import { directiveElse, directiveIf } from '@root/config/directive.config';
...
export default class DataPanel extends React.Component<IComponentProps, any> {
    ...
    tplParser(el: HTMLElement, model) {
        ...
        deepEachElement(el, element => {
            ...
            if (attrs[directiveIf]) {
                let { value: fieldTpl } = attrs[directiveIf];
                ...
                try {
                    ...
                    if (ifResult) {
                        $(element).next().attr(directiveElse) !== undefined && $(element).next().remove();
                    } else {
                        $(element).remove();
                    }

                    $(element).removeAttr(directiveIf);
                    $(element).next().removeAttr(directiveElse);
